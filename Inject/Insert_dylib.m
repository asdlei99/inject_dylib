...
//./insert_dylib --all-yes @executable_path/${libname}.dylib ${binary_path}
...
//        usage();
//    }
    NSURL *dylibURL = [NSURL fileURLWithPath:dylibPath];
    NSURL *appURL = [[NSURL fileURLWithPath:appPath] URLByDeletingPathExtension];
    NSString *binaryPath = [NSString stringWithFormat:@"%@/%@",appPath,appURL.lastPathComponent];
    NSString *dylibDstPath = [NSString stringWithFormat:@"%@/%@",appPath,dylibURL.lastPathComponent];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    [fileManager copyItemAtPath:dylibPath toPath:dylibDstPath error:nil];
    
    const char *lc_name = weak_flag? "LC_LOAD_WEAK_DYLIB": "LC_LOAD_DYLIB";
    
    const char *dylib_path = [[NSString stringWithFormat:@"@executable_path/%@",dylibURL.lastPathComponent] UTF8String];
    const char *binary_path = [binaryPath UTF8String];
    ...
    switch(magic) {
        ...
        case FAT_CIGAM: {
            ...
            if(fails == 0) {
                NSString *log = [NSString stringWithFormat:@"Added %s to all archs in %s\n", lc_name, binary_path];
                [LogHelper logLowLevel:log];
            } else if(fails == nfat_arch) {
                NSString *log = [NSString stringWithFormat:@"Failed to add %s to any archs.\n", lc_name];
                [LogHelper logHighLevel:log];
                success = false;
            } else {
                NSString *log = [NSString stringWithFormat:@"Added %s to %d/%d archs in %s\n", lc_name, nfat_arch - fails, nfat_arch, binary_path];
                [LogHelper logLowLevel:log];
            }
